...
- **База данных:** PostgreSQL — единое хранилище для всех сущностей (датчики, статусы, метаданные). Доступ через `pgx/v5`.
...
Из анализа монолита и целевой архитектуры выделены пять доменов:
...
| **Автоматизация (design-time)** | Создание, хранение и публикация сценариев поведения устройств |
| **Исполнение команд (runtime)** | Обработка входящих событий, оценка триггеров и условий, отправка команд устройствам |
| **Телеметрия и события** | Приём, нормализация, валидация и хранение показаний от устройств |
| **Интеграция с устройствами** | Связь с устройствами разных производителей через их API и протоколы |
